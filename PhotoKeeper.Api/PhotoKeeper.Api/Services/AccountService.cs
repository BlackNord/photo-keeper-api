...
public class AccountService : IAccountService
{
	...
	public AuthenticateResponse Authenticate(AuthenticateRequest model, string ipAddress)
	{
		...
		// authentication is successful so generate jwt and refresh tokens
	...
	public void RevokeToken(string token, string ipAddress)
	{
		...
		revokeRefreshToken(refreshToken, ipAddress, "Token was revoked without replacement.");
		_context.Update(account);
		_context.SaveChanges();
	}
	...
	public void VerifyEmail(string token)
	{
		...
		if (account == null)
			throw new AppException("Verification was failed.");
	...
	public void ResetPassword(ResetPasswordRequest model)
	{
		...
		account.PasswordHash = BCrypt.HashPassword(model.Password);
		account.ResetPasswordTime = DateTime.UtcNow;
	...
	public AccountResponse Update(int id, UpdateRequest model)
	{
		...
		account.UpdationTime = DateTime.UtcNow;
		_context.Accounts.Update(account);
		_context.SaveChanges();
	...
	private Account getAccountByResetToken(string token)
	{
		var account = _context.Accounts.SingleOrDefault(x =>
			x.ResetToken == token && x.ResetTokenExpiresTime > DateTime.UtcNow);
	...
	private void removeOldRefreshTokens(Account account)
	{
		account.RefreshTokens.RemoveAll(x =>
			!x.IsActive &&
			x.CreationTime.AddDays(_appSettings.RefreshTokenTTL) <= DateTime.UtcNow);
	}
	...
	{
		token.CancellationTime = DateTime.UtcNow;
		token.CancelledByIp = ipAddress;
